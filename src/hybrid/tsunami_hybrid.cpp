...
int main(int argc, char* argv[]) {
    ...
    if (rank == 0) {
        ...
        std::cout << "Starting Hybrid MPI+OpenMP Tsunami Simulation (" << N << "x"
                  << N << ") with " << size << " MPI processes and "
                  << num_threads << " OpenMP threads per process...\n";

        if (provided < MPI_THREAD_FUNNELED) {
            std::cout << "Warning: MPI implementation did not provide requested "
        ...
        if (t % OUTPUT_FREQ == 0) {
            ...
            if (rank == 0) {
                const std::string filename =
                    "../../data/ground_truth/hybrid_output_" + std::to_string(t) + ".bin";
    ...
    if (rank == 0) {
        ...
        std::cout << "Hybrid Execution Time (" << size << " MPI processes x "
                  << num_threads << " OpenMP threads): " << max_time
                  << " seconds\n";
    }
